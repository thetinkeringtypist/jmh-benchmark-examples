...
public class PentastickUsingInts implements Puzzle {

    private static final int[] WING_ROW = new int[] {0, 1, 0, 1, 0, 0, 2, 0, 2, 0, 0, 3, 0, 3, 0, 0, 4, 0, 4, 0, 0, 5, 0, 5, 0};
    private static final int[] EDGE_ROW = new int[] {1, 0, 1, 0, 1, 2, 0, 2, 0, 2, 3, 0, 3, 0, 3, 4, 0, 4, 0, 4, 5, 0, 5, 0, 5};
     ...
     */
    public PentastickUsingInts() {
        this(null);
    }

    public PentastickUsingInts(final int[][] initialState) {
        pieces = new int[11][25];
    ...
    public void rotate(final Por por) {
        ...
        // store the corners
        final int tlCorner = pieces[x][y];
        final int blCorner = pieces[rowIndex][y];
        final int trCorner = pieces[x][colIndex];
        final int brCorner = pieces[rowIndex][colIndex];
        ...
        if (crossesBoundary) {
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    ...
                    // Swap
                    int temp = pieces[rowIndex - i][adjustedColIndex];
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    // Swap
                    int temp = pieces[rowIndex - i][colIndex - j];
        ...
        final int[] row = pieces[x + Math.floorDiv(NUM_ROWS_IN_SLICE, 2)];
     ...
     */
    @Override
    public PentastickUsingInts deepCopy() {
        return new PentastickUsingInts(this.pieces);
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof PentastickUsingInts that)) {
            return false;
        }
