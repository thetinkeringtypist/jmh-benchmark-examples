...
public class PentastickUsingFloats implements Puzzle {

    private static final float[] WING_ROW = new float[] {0, 1, 0, 1, 0, 0, 2, 0, 2, 0, 0, 3, 0, 3, 0, 0, 4, 0, 4, 0, 0, 5, 0, 5, 0};
    private static final float[] EDGE_ROW = new float[] {1, 0, 1, 0, 1, 2, 0, 2, 0, 2, 3, 0, 3, 0, 3, 4, 0, 4, 0, 4, 5, 0, 5, 0, 5};
     ...
     */
    public PentastickUsingFloats() {
        this(null);
    }

    public PentastickUsingFloats(final float[][] initialState) {
        pieces = new float[11][25];
    ...
    public void rotate(final Por por) {
        ...
        // store the corners
        final float tlCorner = pieces[x][y];
        final float blCorner = pieces[rowIndex][y];
        final float trCorner = pieces[x][colIndex];
        final float brCorner = pieces[rowIndex][colIndex];
        ...
        if (crossesBoundary) {
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    ...
                    // Swap
                    float temp = pieces[rowIndex - i][adjustedColIndex];
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    // Swap
                    float temp = pieces[rowIndex - i][colIndex - j];
        ...
        final float[] row = pieces[x + Math.floorDiv(NUM_ROWS_IN_SLICE, 2)];
     ...
     */
    @Override
    public PentastickUsingFloats deepCopy() {
        return new PentastickUsingFloats(this.pieces);
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof PentastickUsingFloats that)) {
            return false;
        }
