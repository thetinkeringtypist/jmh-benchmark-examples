...
public class PentastickUsingChars implements Puzzle {

    private static final char[] WING_ROW = new char[] {0, 1, 0, 1, 0, 0, 2, 0, 2, 0, 0, 3, 0, 3, 0, 0, 4, 0, 4, 0, 0, 5, 0, 5, 0};
    private static final char[] EDGE_ROW = new char[] {1, 0, 1, 0, 1, 2, 0, 2, 0, 2, 3, 0, 3, 0, 3, 4, 0, 4, 0, 4, 5, 0, 5, 0, 5};
     ...
     */
    public PentastickUsingChars() {
        this(null);
    }

    public PentastickUsingChars(final char[][] initialState) {
        pieces = new char[11][25];
    ...
    public void rotate(final Por por) {
        ...
        // store the corners
        final char tlCorner = pieces[x][y];
        final char blCorner = pieces[rowIndex][y];
        final char trCorner = pieces[x][colIndex];
        final char brCorner = pieces[rowIndex][colIndex];
        ...
        if (crossesBoundary) {
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    ...
                    // Swap
                    char temp = pieces[rowIndex - i][adjustedColIndex];
            ...
            for (int i = 0; i < middleRows; i++) {
                for (int j = 0; j < NUM_COLS_IN_SLICE; j++) {
                    // Swap
                    char temp = pieces[rowIndex - i][colIndex - j];
        ...
        final char[] row = pieces[x + Math.floorDiv(NUM_ROWS_IN_SLICE, 2)];
     ...
     */
    @Override
    public PentastickUsingChars deepCopy() {
        return new PentastickUsingChars(this.pieces);
    }
    ...
    public boolean equals(final Object obj) {
        ...
        if (!(obj instanceof PentastickUsingChars that)) {
            return false;
        }
